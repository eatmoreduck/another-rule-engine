    ...
    <div>
      ...
      <div style={{ marginBottom: 16 }}>
        <div style={{
          ...
          <Title level={5} style={{ margin: 0 }}>{t('ruleConfig.condition')}</Title>
        ...
        {!showEditor ? (
          <Alert
            type="info"
            showIcon
            message={t('ruleConfig.unconditionalAlert')}
            description={
              <Button
                type="dashed"
                size="small"
      ...
      {showEditor && (
        <div style={{ padding: 12, background: '#fff7e6', borderRadius: 6, border: '1px solid #ffe7ba' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontWeight: 600, color: '#389e0d' }}>{t('ruleConfig.whenMatched')}</span>
            <Select value={action} onChange={handleActionChange} options={ACTION_OPTIONS} style={{ width: 120 }} />
            <Input placeholder={t('ruleConfig.reasonPlaceholder')} value={reason} onChange={handleReasonChange} style={{ flex: 1 }} />
          </div>
        </div>
      )}
      ...
      <div style={{ marginTop: 8, padding: 12, background: '#fafafa', borderRadius: 6 }}>
        <Space>
          <span style={{ fontWeight: 600 }}>
            {!showEditor ? t('ruleConfig.executeAction') : t('ruleConfig.whenNotMatched')}
          </span>
          <Select value={defaultAction} onChange={handleDefaultActionChange} options={ACTION_OPTIONS} style={{ width: 110 }} />
          <Input placeholder={t('ruleConfig.defaultReasonPlaceholder')} value={defaultReason} onChange={handleDefaultReasonChange} style={{ width: 200 }} />
        </Space>
      </div>
    </div>
  );
